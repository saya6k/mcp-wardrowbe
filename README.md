...
| Flag | Env var | Notes |
| --- | --- | --- |
| `--host` | `MCP_BIND_HOST` | Bind address. Default `0.0.0.0`. |
| `--port` | `MCP_BIND_PORT` | Bind port. Default `8080`. |
| `--wardrowbe-url` | `WARDROWBE_URL` | Base URL of the Wardrowbe backend. |
| `--api-key` | `MCP_API_KEY` | Required Bearer token for incoming MCP calls. |
